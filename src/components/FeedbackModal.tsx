...
const STORAGE_KEY = "btl-bina-platform-feedback";
const APP_NAME = "בינה — פלטפורמת AI";
...
async function sendToSheet(entry: FeedbackEntry, page: string): Promise<boolean> {
  try {
    await fetch(SHEET_URL, { method: "POST", mode: "no-cors",
      ...
      body: JSON.stringify({ app: APP_NAME, name: entry.name || "אנונימי",
...
export function FeedbackModal({ open, onClose }: FeedbackModalProps) {
  ...
  useEffect(() => {
    ...
    Promise.all(unsent.map(i => sendToSheet(i, window.location.pathname))).then(r => {
      save(items.map(item => { const idx = unsent.findIndex(u => u.id === item.id); return idx >= 0 && r[idx] ? { ...item, sent: true } : item; }));
    });
  }, [open]);
  ...
  const handleSubmit = async () => {
    ...
    const entry: FeedbackEntry = { id: Date.now(), name: name.trim(), category, severity, text: text.trim(), timestamp: new Date().toISOString(), sent: false };
                ...
                {items.map(fb => (
                  <div key={fb.id} className="bg-gray-50 rounded-lg p-3 text-right border border-gray-200">
                    <div className="flex items-center gap-2 mb-1 flex-wrap justify-end">
                      ...
                      {fb.sent ? <span className="text-xs px-2 py-0.5 rounded-full bg-green-100 text-green-600">✅</span> : <span className="text-xs px-2 py-0.5 rounded-full bg-orange-100 text-orange-600">⏳</span>}
